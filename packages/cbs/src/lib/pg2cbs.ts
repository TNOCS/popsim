...
const sql: { cbs: string[] } = <any>SqlQueries;
const sqlCbs = sql.cbs.join('\n');
 ...
 */
export const queryCbs = async (bbox?: string) => {
  try {
    bbox = bbox || `POLYGON(
      (
    ...
    const res = await query(sqlCbs.replace(/bbox_4326/g, bbox), []);
